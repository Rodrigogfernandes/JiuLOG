...
set SQL_FILE=%~dp0database_setup.sql
...
if not exist "%SQL_FILE%" (
  ...
  echo → Certifique-se de que "database_setup.sql" esta na raiz do projeto.
  pause
  exit /b 1
)
...
if %ERRORLEVEL% EQU 0 (
  ...
  echo   3) Ou importar manualmente via phpMyAdmin (database_setup.sql)
  pause
  exit /b 1
)
